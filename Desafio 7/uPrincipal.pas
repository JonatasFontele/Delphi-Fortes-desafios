...
{
  A padaria XYZ vende uma certa quantidade de pães franceses e uma quantidade de
  broas a cada dia.
  Cada pãozinho custa R$ 0,75 e a broa custa R$ 4,50.
  ...
  Você foi contratado a desenvolver uma aplicação que acumule as vendas do dia e ao
  fechar o caixa calcule as quantidades de pães e de broas e os dados solicitados.
}
...
begin
  try
    ...
    begin
      ...
      RichEdt.Lines.Add('Preço: ' + CurrToStrF(vDict.Items[I].Preco, ffCurrency, 2));
      RichEdt.Lines.Add('');
    end;
    I := I + 1;
  finally
...
begin
  ...
  begin
    ...
    if Produto.Nome = 'Pãozinho' then
      Inc(Paezinhos)
    else if Produto.Nome = 'Broa' then
      Inc(Broas);
  end;
  ShowMessage('Pãezinhos: ' + IntToStr(Paezinhos) + #13 +
              'Broas: ' + IntToStr(Broas) + #13 +
              'Total arrecadado: ' + CurrToStrF(ValorTotal, ffCurrency, 2) + #13 +
              'Guardar na poupança: ' + CurrToStrF(ValorTotal * 0.1, ffCurrency, 2));
end;
